...
class OpenAIBot(Bot):
    ...
    def reply_text(self, query, user_id, retry_count=0):
        """调用openAI api Completion 接口，生成回复文字

        Args:
            query (String): user's prompt
            user_id (String): weichat user id
            retry_count (Numbers, optional): 重复次数. Defaults to 0.
        ...
        try:
            ...
            res_content = response.choices[0]['text'].strip().replace(
                '<|endoftext|>', '')
            logger.info("[OPEN_AI] reply=%s", res_content)
            return res_content
