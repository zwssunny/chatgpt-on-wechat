...
TOKEN_PATH = TmpDir().path() + ".baiduSpeech_token"
...
class baiduSpeech(object):
    ...
    def fetch_token(self):
        ...
        try:
            ...
            if "access_token" in result.keys() and "scope" in result.keys():
                if "brain_enhanced_asr" not in result["scope"].split(" "):
        ...
        try:
            with open(TOKEN_PATH, "r") as f:
                ...
                if not access_token:
                    ...
                    time.time() -
                    access_token["expires_in"] - access_token["get_time"]
                    >= 0
                ):
                    return self.fetch_token()
                else:
    ...
    def asr(self, pcm, file_type, sample_rate, dev_pid):
        ...
        params = {"token": self.token, "dev_pid": dev_pid}

        try:
            req = requests.post(asr_url, params=params,
                                headers=headers, data=pcm)
